...
import { EaiiNavigationItem, EaiiVerticalNavigationAppearance, EaiiVerticalNavigationMode, EaiiVerticalNavigationPosition } from '@eaii/components/navigation/navigation.types';
...
@Component({
    selector       : 'eaii-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : eaiiAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'eaiiVerticalNavigation'
})
export class EaiiVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: EaiiVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: EaiiVerticalNavigationMode = 'side';
    @Input() name: string = this._eaiiUtilsService.randomId();
    @Input() navigation: EaiiNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: EaiiVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<EaiiVerticalNavigationAppearance> = new EventEmitter<EaiiVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<EaiiVerticalNavigationMode> = new EventEmitter<EaiiVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<EaiiVerticalNavigationPosition> = new EventEmitter<EaiiVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<EaiiNavigationItem> = new ReplaySubject<EaiiNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<EaiiNavigationItem> = new ReplaySubject<EaiiNavigationItem>(1);
    ...
    private _eaiiScrollbarDirectives!: QueryList<EaiiScrollbarDirective>;
    private _eaiiScrollbarDirectivesSubscription: Subscription;
    ...
    {
        return {
            'eaii-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`eaii-vertical-navigation-appearance-${this.appearance}`]: true,
            'eaii-vertical-navigation-hover'                          : this._hovered,
            'eaii-vertical-navigation-inner'                          : this.inner,
            'eaii-vertical-navigation-mode-over'                      : this.mode === 'over',
            'eaii-vertical-navigation-mode-side'                      : this.mode === 'side',
            'eaii-vertical-navigation-opened'                         : this.opened,
            'eaii-vertical-navigation-position-left'                  : this.position === 'left',
            'eaii-vertical-navigation-position-right'                 : this.position === 'right'
        };
    }
    ...
    /**
     * Setter for eaiiScrollbarDirectives
     */
    @ViewChildren(EaiiScrollbarDirective)
    set eaiiScrollbarDirectives(eaiiScrollbarDirectives: QueryList<EaiiScrollbarDirective>)
    {
        // Store the directives
        this._eaiiScrollbarDirectives = eaiiScrollbarDirectives;

        // Return if there are no directives
        if ( eaiiScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._eaiiScrollbarDirectivesSubscription )
        {
            this._eaiiScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._eaiiScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    eaiiScrollbarDirectives.forEach((eaiiScrollbarDirective) => {
                        eaiiScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.eaii-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._eaiiScrollbarDirectives.forEach((eaiiScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !eaiiScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    eaiiScrollbarDirective.scrollToElement('.eaii-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: EaiiNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: EaiiNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('eaii-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('eaii-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('eaii-vertical-navigation-aside-overlay');
