...
import { ScrollbarGeometry, ScrollbarPosition } from '@eaii/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[eaiiScrollbar]',
    exportAs: 'eaiiScrollbar'
})
export class EaiiScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_eaiiScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() eaiiScrollbar: boolean = true;
    @Input() eaiiScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'eaiiScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.eaiiScrollbar = coerceBooleanProperty(changes.eaiiScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.eaiiScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'eaiiScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.eaiiScrollbarOptions.currentValue);
