...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--eaii-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--eaii-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--eaii-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--eaii-${e(key)}`, value], [`--eaii-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`eaii.customProps.background.${colorScheme}`);
            const foreground = theme(`eaii.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--eaii-${e(key)}`, value], [`--eaii-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--eaii-${e(key)}`, value], [`--eaii-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
